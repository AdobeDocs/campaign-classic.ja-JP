...
これらのフィルターは、ターゲット選択ステージで配信アシスタントを介して直接アクセスすることもできます（配信の作成について詳しくは [ この節 ](../../delivery/using/creating-an-email-delivery.md) を参照してください）。 アプリケーションフィルターを作成するには、次の操作をおこなうことができます。
...
* 「**[!UICONTROL 詳細設定パラメーター]**」リンクを使用すると、追加設定を定義できます。特に、SQL テーブルをフィルターに関連付けて、そのテーブルを共有するすべてのエディターでそのフィルターを共通にすることができます。
  ...
  このオプションは、配信アシスタントで提供される「配信の受信者」フィルターおよび「フォルダーに属する配信の受信者」フィルターがオーバーロードできない場合に有効になります。
