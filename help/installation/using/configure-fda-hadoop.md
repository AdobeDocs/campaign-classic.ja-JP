...
description: FDAでHadoopへのアクセスを設定する方法について説明します
...
| fileFormat | PARQUET | ワークテーブルのデフォルトのファイル形式です。 |


## Hadoop 2.1の設定 {#configure-access-hadoop-2}

Hadoop 2.1に接続する必要がある場合は、[Windows](#for-windows)または[Linux](#for-linux)の次の手順に従ってください。
...
1. ODBC DataSource Administrator ツールを実行して、DSN （Data Source名）を作成します。 ユーザーが変更できる Hive 用 System DSN サンプルが提供されています。
...
1. [この節](#hadoop-external)で詳しく説明しているように、Hadoop外部アカウントを作成します。

### Linux用Hadoop 2.1 {#for-linux}
...
1. HortonWorksからApache Hive用ODBC ドライバーをダウンロードしてインストールします：[https://www.cloudera.com/downloads.html](https://www.cloudera.com/downloads.html)。
