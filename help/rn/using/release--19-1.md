...
## ![](assets/do-not-localize/limited_2.png) リリース 19.1.7 - ビルド 9036 {#release-19-1-7-build-9036}

_2020年9月15日_

**強化点**

* nlsrvmodのクラッシュを修正するために、Apache 2.4のスレッドでのnlsrvmodの使用を改善。
* Azure外部アカウントとSSL暗号化でFile Transferアクティビティを使用する場合の問題を修正しました。 接続は、HTTPSではなくHTTPを使用して実行されました。 （NEO-26720）



* 配信のプロパティで、「電子メールを **[!UICONTROL アーカイブ]** 」オプションの名前が「 **[!UICONTROL 電子メールBCC]** 」に変更され、ユーザーの使い勝手が向上しました。
* ラベルやカテゴリを取得できないURLキャッシュメカニズムに関する問題を修正しました。
* 電子メール配信で、（不適切なASCII文字制御が原因で）ミラーページURLが正しく定義されない問題を修正しました。 （NEO-26084）
* catalina.properties の jarsToSkip リストが更新され、使用されなくなった jar ファイル（iOS 通知）への参照が削除されました。
* アップグレード後の発行後に発生しなかった回帰の問題を修正しました。
* PDFに書き出すとすぐに使用できる配信レポートが切り捨てられて表示されていた問題を修正しました。 （NEO-25757）
* トラッキングURLからリダイレクトする際にエンコーディングパラメーターの値が削除される（日本語文字に影響）問題を修正しました。 （NEO-25637）
* 許可されるべき場合に、パーソナライズされたドメインがブロックされる未署名のリンクが発生する問題を修正しました。（NEO-25210）
* ワークフローの回帰影響の計算済みフィールドでワークフローが失敗する問題を修正しました。 （NEO-25194）
* 一部のAPI呼び出しの実行を妨げる可能性があるMicrosoft Dynamics（バージョン8.2から）の互換性の問題を修正しました。 （NEO-24528）
* ACSコネクタ機能を使用した場合にCampaign Standardインスタンスへの接続ができない（FOH/FOH2接続の誤った管理）問題を修正しました。 （NEO-23433）
* データベースのエンコーディングの問題が原因で、Webサーバーが常に再起動する、データベース接続の問題を修正しました。 これは過剰消費につながる可能性がある。 （NEO-23264）
...
* 初期化されていない変数(m_pCurlReader)が原因で、Azure Blobストレージ外部アカウントを使用してファイル転送アクティビティのファイルをアップロードできない問題を修正しました。 （NEO-13717）
...
* **** スケジューラーワークフローアクティビティで時間を設定すると、誤ったタイムゾーンが選択される問題を修正しました。
...
* コマンドラインを使用したmidEmitterパッケージのインストールに関する問題を修正しました。
* Microsoft Dynamics を使用した AC コネクタ内の OAuth 資格情報をサポートするための新しい認証オプションが追加されました。（NEO-11982）
* UUID(Unique Universal Identifier)管理で、クエリおよびデータ読み込みワークフローのアクティビティがHiveFDAで失敗する問題を修正しました。
* Oracleの回帰が修正され、アップグレード後に一部の関数が無効と見なされる問題を修正しました。 （NEO-12759）



* スケジューラーワークフローアクティビティで時間を設定した場合に誤ったタイムゾーンが選択される問題が修正されました。
