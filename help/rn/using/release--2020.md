...
* nmsBroadlogId シーケンスに対して、負の ID 範囲のサポートが追加されました。このビルドでは、nmsBroadlogId シーケンスの min_value が負の範囲を含むように調整されます。負の ID を使用できない厳密なユースケースがある場合は、シーケンスの min_value を 1 に戻してください。
