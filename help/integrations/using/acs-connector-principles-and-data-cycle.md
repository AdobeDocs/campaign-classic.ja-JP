...
# ACS コネクタの基本を学ぶ{#acs-connector-gs}
...
* **[!UICONTROL 配信 ID]**
* **[!UICONTROL メール配信ログ]**
* **[!UICONTROL メールトラッキングログ]**

配信 ID およびメールログをレプリケートすることで、Campaign v7 から v7 受信者の配信の履歴およびトラッキングデータにアクセスできます。

>[!IMPORTANT]
>
>Campaign Standard から Campaign v7 にレプリケートされるのは、メール配信ログとトラッキングログのみです。
...
* **Campaign Standard の基本的な受信者データ**：ACS コネクタは、Campaign Standard に適した受信者データをレプリケートします。 このデータには、受信者の名前、住所、メールアドレス、携帯電話番号、自宅電話番号、その他関連のある連絡先情報が含まれます。 Campaign v7 で使用できる追加の受信者フィールドおよびカスタムターゲティングテーブルがワークフローで重要な場合、コンサルタントにお問い合わせください。
...
* **[!UICONTROL `[ACS] New delivery log replication`]**（newRcpDeliveryLogReplication）：この増分ワークフローは、配信 ID、メール配信ログおよびメールトラッキングログを Campaign Standard から Campaign v7 にレプリケートします。 ここでは、Campaign Standard から Campaign v7 の（nms:recipients 以外の定義する）特定のテーブルの一部であるプロファイルに送信された配信のみが考慮されます。
