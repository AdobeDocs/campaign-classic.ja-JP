...
* エラーを回避するには、 **ヒートマップサービスのデータを収集** (collectDataHeatMapService) ワークフローがデフォルトで停止されるようになりました。 （NEO-33959）
...
* MID インスタンスで配信ログのステータスを更新する際に、 `FeatureFlag_GZIP_Compression` 」オプションが有効になっている。 （NEO-49183）
...
* イベントタイプで **Message Center の同期** (mcSynch) ワークフロー
* を追加する際にエラーが発生する可能性がある問題を修正しました。 **開封した受信者**  (estimatedRecipientOpen) 指標を **クエリ** ワークフローアクティビティ。 （NEO-46665）
