...
# SMSトラブルシューティング{#troubleshooting-sms}
...
[`BIND PDU`で使用できるフィールドは&lt;a0/>](../../delivery/using/sms-protocol.md#information-pdu)の&lt;a0/>の各種PDU&lt;a1/>の情報を参照してください。通常は`address_range`に短いコードを追加しますが、プロバイダの特別なサポートが必要です。 複数の短縮コードを個別にルーティングする方法については、担当者にお問い合わせください。
...
* ミッドソーシング環境で問題が発生した場合は、配信とブロードログが適切に作成され、ミッドソーシングサーバーで更新されていることを確認してください。 そうでない場合は、SMSの問題ではありません。

* ミッドサーバー上で動作し、SMSが正しく送信され、マーケティングインスタンスが正しく更新されない場合は、中間同期の問題が発生する可能性があります。
...
* 接続は1時間未満続きます。 Adobe Campaign Classic送信機接続は、Adobe Campaign ClassicMTAの動作の仕方が原因で例外です。
...
* プロバイダが`TCP FIN`または`TCP RST packet`を送信して接続を閉じた場合は、プロバイダに詳しい情報を問い合わせてください。
...
* 接続が安定していることを確認します。 SMPP接続は、Adobe Campaign Classic上の送信機を除き、少なくとも1時間は継続的に起動し続ける必要があります。 [不安定な接続に関する問題](../../delivery/using/sms-protocol.md#issues-unstable-connection)を参照してください。

* MTAを再起動すると、MTが再び送信され、しばらくの間作業が行われる場合、接続が不安定なためスロットリングが発生する可能性があります。 [不安定な接続に関する問題](../../delivery/using/troubleshooting-sms.md#issues-unstable-connection)を参照してください。
...
* `BIND/UNBIND`を多く見ると、不安定なつながりを持ちます。 重複メッセージの問題を解決する前に、[不安定な接続の問題](../../delivery/using/troubleshooting-sms.md#issues-unstable-connection)の節を参照してください。
...
* Adobe Campaignが`DELIVER_SM_RESP PDU`に正常に応答したことをタイムリーに確認します。 Adobe Campaign Classicでは、SRが`providerMsgId`テーブルに挿入され、SMSプロセスによる遅延処理が行われていることが保証されます。
...
`DELIVER_SM PDU`がAdobe Campaign Classic拡張SMPPコネクタで認識されたが、broadLogが正しく更新されない場合は、「[MT、SRおよびbroadlogエントリの一致](../../delivery/using/sms-protocol.md#matching-mt)」で説明されているID調整プロセスを確認します。

無効なSR以外のすべてを修正しても、プロバイダのバッファに残っている場合は、「Invalid ID acknowledge count」オプションを使用してスキップできます。 これは注意して使用し、バッファがクリーンになった後、できるだけ早く0にリセットする必要があります。
...
* `DELIVER_SM PDU`が表示された場合は、Adobe Campaignが`DELIVER_SM_RESP PDU`に成功したことを確認してください（コード0）。 このRESPは、すべての処理ロジックがAdobe Campaignによって適用されていることを保証します(自動応答および許可/ブロックリスト)。 そうでない場合は、SMSプロセスログでエラーメッセージを検索します。

* 自動返信が有効な場合は、`SUBMIT_SM`がプロバイダーに送信されたことを確認します。 見つからない場合は、必ずSMSプロセスログにエラーメッセージが表示されます。
...
* 電話番号の形式が、強制隔離テーブルと配信ログで完全に同じであることを確認します。 それ以外の場合は、この[セクション](../../delivery/using/sms-protocol.md#automatic-reply)を参照してください。
...
* **問題が発生した場合に、詳細SMPP** メッセージを有効にします。ほとんどのSMS問題は、これなしでは解決できません。
...
その他の状況では、詳細なSMPPメッセージを最初に分析し、詳細ログに情報がない場合にのみネットワークキャプチャを要求してください。
...
1. **外部アカウント**&#x200B;で、**ログファイル**&#x200B;の詳細SMPPトレースを有効にするをオンにします。
1. 10分待って、サーバーが外部アカウントを再読み込みするまで待ちます。
