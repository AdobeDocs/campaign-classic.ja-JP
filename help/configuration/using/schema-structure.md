...
スキーマの基本構造について以下に説明します。

## データスキーマ  {#data-schema}

`<srcschema>`の場合、構造は次のとおりです。
...
スキーマのエントリポイントは、その主な要素です。 メイン要素はスキーマと同じ名前なので、識別は容易です。また、メイン要素はルート要素の子でなければなりません。 コンテンツの記述は、この要素から始まります。
...
メイン要素に続く&#x200B;**`<attribute>`**&#x200B;要素と&#x200B;**`<element>`**&#x200B;要素は、XML構造内のデータ項目の場所と名前を定義するために使用されます。
...
* 各 **`<element>`** と **`<attribute>`** は、**name** 属性を介して名前で識別する必要があります。
  ...
  >[!IMPORTANT]
  >
  >要素の名前は簡潔で、好ましくは英語で、XML命名規則で許可されている文字のみを含める必要があります。
...
* **uuid**: GUIDをサポートする「一意識別子」フィールド（Microsoft SQL Serverでのみサポート）。

  >[!NOTE]
  >
  >Microsoft SQL Server以外のRDBMSに&#x200B;**uuid** フィールドを含めるには、`the newuuid()`関数を追加し、デフォルト値で完了する必要があります。
...
### Adobe Campaign/DBMS データの種類のマッピング {#mapping-the-types-of-adobe-campaign-dbms-data}

次の表に、様々なデータベース管理システム用にAdobe Campaignで生成されるデータの種類のマッピングを示します。
...
データスキーマの&#x200B;**`<elements>`**&#x200B;要素と&#x200B;**`<attributes>`**&#x200B;要素は、様々なプロパティで強化できます。 現在の要素を説明するためにラベルを入力できます。
...
* **desc** プロパティを使用すると、詳細な説明を入力できます。
  ...
  説明は、Adobe Campaign クライアントコンソールのメインウィンドウのステータスバーの入力フォームに表示されます。
...
値は、XPath 言語に準拠した式である必要があります。 詳しくは、「[XPath](../../configuration/using/schema-structure.md#referencing-with-xpath)での参照」を参照してください。
...
デフォルト値をフィールドにリンクするには、`<default>`または`<sqldefault>` フィールドを使用できます。
...
**userEnum** プロパティを使用すると、このフィールドを使用して入力された値を保存および表示するためのオープン列挙を定義できます。
...
>[!NOTE]
>
>Adobe Campaign クライアントコンソールで、エクスプローラーの&#x200B;**[!UICONTROL 管理/列挙]** フォルダーを参照して、列挙を管理します。
...
***dbenum** プロパティを使用すると、プロパティが&#x200B;**enum** プロパティと類似している列挙を定義できます。
...
## XPathでの参照 {#referencing-with-xpath}
...
* **@email + &#39;（&#39;+@created+&#39;）&#39;**：作成日に追加された電子メールアドレスの値をかっこで囲んで文字列を作成します（文字列タイプの場合は、定数を引用符で囲みます）。
