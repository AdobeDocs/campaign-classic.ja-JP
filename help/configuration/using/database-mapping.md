...
主な欠点は、XMLフィールドのインデックス作成やフィルタリングが不可能である点です。
...
* 一意のインデックスが各キー定義に対して暗黙的に宣言されます。 値&quot;true&quot;に&#x200B;**noDbIndex**&#x200B;属性を追加することで、キーにインデックスを作成できなくなります。
...
>[!NOTE]
>
>テーブルマッピング中にキーが作成される（標準またはFDA）、Adobe Campaignは一意のインデックスを見つけます。
...
ほとんどのAdobe Campaignテーブルのプライマリキーは、データベースエンジンによって自動生成される32ビット長の整数です。 キー値の計算は、データベース全体で一意の数を生成するシーケンス（デフォルトでは&#x200B;**XtkNewId** SQL関数）に依存します。 レコードの挿入時に、キーの内容が自動的に入力されます。

増分キーの利点は、テーブル間の結合に対して変更不可能なテクニカルキーを提供することです。 また、このキーは2バイトの整数を使用するので、メモリをあまり占有しません。
...
ACC 18.10以降では、 **XtkNewId**&#x200B;は標準のスキーマのシーケンスのデフォルト値ではなくなりました。 これで、スキーマを作成したり、専用のシーケンスを使用して既存のスキーマを拡張したりできるようになりました。
...
>[!NOTE]
>
>Adobe Campaignスキーマで参照されるシーケンス（例：**NmsTrackingLogId**）は、SQL関数に関連付ける必要があります。この関数は、パラメーター内のIDの数をコンマで区切って返します。 この関数は、**GetNew** XXX **Ids**&#x200B;と呼ばれる必要があります。ここで、**XXX**&#x200B;はシーケンスの名前です（例えば、**GetNewNmsTrackingLogIds**）。 **datakit/nms/eng/sql/**&#x200B;ディレクトリのoracleに付属する&#x200B;**postgres-nms.sql**、**mssql-nms.sql**&#x200B;または&#x200B;**datakit/nms/eng/sql**&#x200B;ファイルを表示して、「NmsTracking」の例を復元します各データベースエンジンに対するLogIdのシーケンス作成。
...
キーとそのインデックスの定義に加えて、自動生成されたプライマリキーを含めるために、拡張スキーマに「id」という数値フィールドが追加されました。
...
* ![](assets/join_with_campaign1n.png)：一対多のカーディナリティ。例えば、受信者と購読テーブルなどがあります。受信者は、購読テーブルの複数のオカレンスに関連付けることができます。
...
FDAテーブルについて詳しくは、[外部データベースへのアクセス](../../installation/using/about-fda.md)を参照してください。
