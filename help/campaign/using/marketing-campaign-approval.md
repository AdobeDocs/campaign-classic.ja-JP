...
レビュー担当者に指名されている Adobe Campaign オペレーターに通知メッセージが送信されて、承認リクエストを通知します。 レビュー担当者が承認の&#x200B;**適切な権限**&#x200B;を持っていること、およびレビュー担当者のセキュリティゾーンが正しく定義されていることを確認します。 [詳細情報](#selecting-reviewers)。

承認手順については、[こちらの節](#checking-and-approving-deliveries)を参照してください。
...
この情報は、キャンペーンの承認ログにも表示されます。これらのログには、「**[!UICONTROL 編集／追跡／承認]**」タブからアクセスします。
...
### 承認プロセスの選択 {#choosing-the-processes-to-be-approved}

承認フェーズは、キャンペーンに関連付けられたテンプレートで定義されます。 承認する要素をテンプレートから選択し、その承認を担当する Adobe Campaign オペレーターを指定する必要があります。 キャンペーンテンプレートの詳細については、[こちらの節](../../campaign/using/marketing-campaign-templates.md#campaign-templates)を参照してください。
...
>[!CAUTION]
>
>コンテンツを承認するには、配達確認サイクルが必須です。 配達確認を使用すると、情報やパーソナライゼーションデータの表示を承認し、リンクが機能していることを確認できます。 配達確認を作成する方法については、[こちらの節](../../delivery/using/steps-validating-the-delivery.md#sending-a-proof)を参照してください。
>
