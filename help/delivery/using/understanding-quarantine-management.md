...
同様に、電子メールアドレスが強制隔離されているプロファイルは、プロファイルを更新して新しいアドレスを入力できるので、再び配信アクションのターゲットになる可能性があります。

一方、**ブロックリスト**&#x200B;への登録では、登録されたプロファイルはそれ以降、どの配信でもターゲットにはなりません（例：購読解除（オプトアウト）後）。

>[!NOTE]
>
>SMS 配信からオプトアウトするために、ユーザーが「STOP」のようなキーワードを使って SMS メッセージに返信しても、そのユーザーのプロファイルは、メールのオプトアウトプロセスのようにブロックリストには登録されません。強制隔離されるのはプロファイルの電話番号なので、ユーザーは引き続き E メールメッセージを受信できます。
