...
同様に、E メールアドレスが強制隔離されているプロファイルは、プロファイルを更新して新しいアドレスを入力でき、再び配信アクションのターゲットにされる可能性があります。

一方、**ブロックリスト**&#x200B;への登録では、登録されたプロファイルが、購読解除（オプトアウト）後のように、それ以降はどのような配信のターゲットにもならなくなります。

>[!NOTE]
>
>ユーザーが SMS 配信をオプトアウトするために「STOP」などのキーワードで SMS メッセージに返信した場合、電子メールのオプトアウトプロセスとは異なり、ユーザーのプロファイルはに追加されませブロックリストん。 強制隔離されるのはプロファイルの電話番号なので、ユーザーは引き続き E メールメッセージを受信できます。
