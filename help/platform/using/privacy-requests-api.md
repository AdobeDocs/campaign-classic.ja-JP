...
1. **xtk:session** WSDLを次のURLからダウンロードしてください：**`<server url>`/nl/jsp/schemawsdl.jsp?schema=xtk:session**。
...
1. WSDLを&#x200B;**`<server url>`/nl/jsp/schemawsdl.jsp?schema=nms:privacyRequest**&#x200B;のURLからダウンロードします。
...
1. **[!UICONTROL CreateRequestByName]** を使用して、特定のプライバシーリクエストを作成します。
   ...
   * **nms:gdprRequest** スキーマの&#x200B;**queryDef**&#x200B;を使用して、アクセス要求のステータスを確認できます。
   * **nms:gdprRequestData** スキーマの&#x200B;**queryDef**&#x200B;を使用して、アクセス要求の結果を取得できます。
