...
Campaign Classic v7 のホスティングモデル、デプロイメントの違い、移行パスに関する回答を確認します。 これらの質問では、インフラストラクチャの選択肢と関連する責任に焦点を当てています。
...
ビルドアップグレードのガイダンスと関連する FAQ については、[&#x200B; ビルドアップグレードに関する FAQ](faq-build-upgrade.md) および [&#x200B; ビルドアップグレードに関するドキュメント &#x200B;](../../production/using/build-upgrade.md) を参照してください。
...
+++ ホスト環境のお客様は、インスタンス設定をどこで管理できますか？{#where-to-manage-instance-settings}

[Campaign コントロールパネル](https://experienceleague.adobe.com/docs/control-panel/using/control-panel-home.html?lang=ja){target="_blank"} は、Adobe Campaignの製品管理者が設定を管理し、各インスタンスの使用状況を追跡するのに役立ちます。 直感的なインターフェイスにより、主要なアセットを監視し、IP許可リストの更新、SFTP ストレージのモニタリング、鍵の管理などの管理タスクを実行できます。
...
コミュニティフォーラムやAdobe サポートへのリンクなど、Campaign Classic v7 の主要なドキュメント、FAQ およびサポートチャネルについて説明しています。
...
+++ Campaign Classic v7 に対するコミュニティまたはAdobeのサポートを受けるにはどうすればよいですか？{#where-to-get-support-v7}
