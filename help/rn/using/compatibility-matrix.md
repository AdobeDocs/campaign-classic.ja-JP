...
Campaign v7.4 以降、Campaign SDK は[非推奨](deprecated-features.md)です。 既存の実装を AEP Mobile SDK にスムーズに移行できるよう、以下のオペレーティングシステムでは引き続き使用可能です<!--, using the associated [mobile SDK](../../delivery/using/integrating-campaign-sdk-into-the-mobile-application.md)-->。
