...
次の例では、**yahoo** （yahoo.fr、yahoo.com、yahoo.beなど）または&#x200B;**ymail** （ymail.com、ymail.euなど）で始まるすべてのメールドメインを示します。 は、**YAHOO!**&#x200B;というラベル、および&#x200B;**rocketmail.com** ドメインのアドレスにグループ化されます。
...
* 表示する測定の種類：パーセント、合計、小数点数など（[表示される測定の種類の変更](#changing-the-type-of-measure-displayed)を参照）。
