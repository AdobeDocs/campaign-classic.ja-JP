...
>[!NOTE]
>
>暫定カレンダーでスケジュールされていない限り、テストモードの配信が互いに影響を与えることはありません（分散型マーケティングのキャンペーンを評価する場合など）。\
...
* **[!UICONTROL シミュレーションの概要]**：このレポートは、様々な配信でシミュレーションから除外された母集団を表示します。
