...
* データ読み込み(RDBMS)ワークフローアクティビティで **リンクテーブル****** 追加フィールドが空の場合に発生するクラッシュの問題を修正しました。 （NEO-12213）
...
* 初期化されていない変数が原因で、電子メール内の追跡対象URLを「 **テキストコンテンツ** 」タブからチェック解除した場合に発生する可能性があるコンソールのクラッシュの問題を修正しました。 （NEO-13545）
