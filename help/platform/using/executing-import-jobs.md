...
インポートアシスタントを使用すると、インポートの設定、オプション（データ変換など）の定義、ローンチの実行を行うことができます。 これは、インポートのタイプ（簡易または複数）やオペレーターの権限によって内容の異なる一連の画面です。

新しいインポートジョブを作成すると、インポートアシスタントが表示されます（[ インポートジョブとエクスポートジョブの作成 ](../../platform/using/creating-import-export-jobs.md)。
...
読み込みアシスタントを起動するときは、まずテンプレートを選択する必要があります。 例えば、ニュースレターを受け取った受信者のインポートを設定するには、以下の手順に従います。
...
1. 該当するフィールドでインポートタイプを選択します。インポートには、1 つのファイルのみをインポートする「**[!UICONTROL 簡易インポート]**」と、1 回の実行で複数のファイルをインポートする「**[!UICONTROL 複数インポート]**」の 2 つのタイプがあります。
   ...
   複数のインポートの場合は、インポートアシスタントの最初の画面にある **[!UICONTROL インポートの種類]** ドロップダウンリストから **[!UICONTROL 複数のインポート]** を選択します。
...
1. **[!UICONTROL 追加]**&#x200B;をクリックして、インポートするフィールドを指定します。
   ...
   ファイルを追加するたびに、「インポートするファイル **[!UICONTROL ウィザードの画面が表示され]** す。 [ 手順 2 - Source ファイルの選択 ](#step-2---source-file-selection) を参照し、ウィザードの手順に従って、簡単なインポートの場合と同様にインポートオプションを定義します。
...
インポートアシスタントの紐付け手順では、ファイルのデータとデータベース内の既存のデータの紐付けのモードを定義し、ファイルデータとデータベースデータ間の優先度ルールを設定できます。 設定ウィンドウは次のように表示されます。
...
インポートアシスタントの次の手順では、データのインポート先となるフォルダーを選択または作成し、インポートされた受信者を（新規または既存の）リストと自動的にマッピングし、受信者をサービスに登録できます。
...
アシスタントの最後の手順では、データのインポートを開始できます。 そのためには、「**[!UICONTROL 開始]**」ボタンをクリックします。
