...
* 設定時に **Web 分析** 外部アカウント。これにより、エラーが発生した場合でも、統合ステータスに常に「統合成功」と表示されていました。 （NEO-36672）
* 負の ID がある場合のシーケンス ID のメカニズムに関連するいくつかのポストアップグレードエラーを修正しました。 （NEO-43205、NEO-42846、NEO-42845）
* を使用する際に発生していた問題を修正しました。 **Web 分析** 繰り返し配信と連続配信を含む外部アカウント。これにより、外部アカウントからのデータが部分的に失われていました。 （NEO-38548）
* NmsActiveContact テーブルを更新する際にアップグレード後のパフォーマンスが低下する問題を修正しました。 （NEO-43206）
* 標準のフォルダーを **管理** ノードを別の場所に移動します。 （NEO-42875）
* を使用する際に発生していた問題を修正しました。 **データを更新** Google Cloud 外部データベースからの受信者データで受信者スキーマを更新できない可能性のあるワークフローアクティビティ。 （NEO-42343）
* Adobe Analyticsコネクタに関連するポストアップグレード中の問題を修正しました。 （NEO-43318、NEO-38136）
* ポストアップグレード時に「VALUE_TO_CHANGE」という問題が発生して上書きされた CUID を修正しました。 （NEO-43267）
