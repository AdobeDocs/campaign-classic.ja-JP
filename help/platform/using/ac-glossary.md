...
詳細情報：[Campaign](../../campaign/using/designing-marketing-campaigns.md)。
+++
...
配信品質は、バウンスしたりスパムと見なされることなく受信者のインボックスに到達するキャンペーンの成否を測定する手法です。より正確には、電子メールの配信品質とは、個人のメールアドレスを介して、メッセージが短時間で、コンテンツや形式の面で期待される品質で宛先に届くかどうかを決定する一連の特性を意味します。
...
配信の概要は、会社が作成した、特定のキャンペーン用の構造化された要素（ドキュメント、店舗、プロモーションクーポンなど）のセットです。 これは、ダイレクトメール配信のコンテキストで使用されます。
...
分散型マーケティングのアドオンにより、Campaign オペレーターは、セントラルエンティティ（本社、マーケティング部門など）とローカルエンティティ（販売店、地域代理店など）の間でキャンペーンを実装するための共同作業ワークスペースを使用できます。 この連携のベースとなるのは、**Campaign パッケージのリスト**&#x200B;と呼ばれる共有ワークスペースで、主にセントラルエンティティで作成されたキャンペーンのテンプレートやインスタンスが、ローカルエンティティに提供されます。
...
エンリッチメントアクティビティの詳細は[こちら](../../workflow/using/enrichment.md)。
+++
...
疲労管理の詳細は[こちら](../../campaign-opt/using/pressure-rules.md)。
+++
...
ファイル抽出の承認の詳細は[こちら](../../delivery/using/validating.md)。
+++
...
フィルタリングディメンションの詳細は[こちら](../../workflow/using/building-a-workflow.md#targeting-and-filtering-dimensions)。
+++
...
ヒートマップの詳細は[こちら](../../workflow/using/heatmap.md)。
+++
...
MRM の詳細は[こちら](../../mrm/using/about-marketing-resource-management.md)。
+++
...
[目標](../../mrm/using/creating-and-managing-tasks.md#expenses-and-revenues)の詳細情報。
+++
...
オファープレゼンテーションルールは、オファー環境で参照されるタイポロジルールであり、受信者の提案履歴を考慮して特定のオファーを除外できます。
...
オファーシミュレーションを使用すると、オペレーターは、実際にオファーを送信する前に、定義した範囲（配信日、ターゲットセグメント、オファー数、テーマなど）でオファー分布をテストできます。 これを使用すると、オファーの優先度と実施要件ルールを調整して、オファーの有効性を最大限に高めることができます。
...
[パレット](../../workflow/using/building-a-workflow.md#adding-and-linking-activities)の詳細情報。
+++
...
Personalization変数は、受信者の情報に基づいて様々なテキストを様々な受信者に表示できる、配信のコードの一部です。 これらのフィールドは、パーソナライゼーションフィールドまたはブロックとして実装できます。
...
[定義済みフィルター](../../platform/using/creating-filters.md#filtering-recipients)の詳細情報。
+++
...
Adobe Campaignでは、受信者とは、配信（メール、SMS など）を顧客に送信する際に対象とするデフォルトのプロファイルのことです。 データベースに格納された受信者データを使用すると、ターゲットをフィルタリングし、パーソナライゼーションデータを追加できます。通常、これは個人情報、連絡先情報、デモグラフィック情報およびトランザクション情報ですが、マーケティングや分析をサポートするあらゆる種類の情報が対象となります。
...
シードアドレスは、定義されたターゲット条件に一致しない受信者を配信のターゲットにする場合に使用されます。これにより、配信スコープ外の受信者が他のターゲット受信者と同様に配信を受信することができます。受信者データベースの不正使用を検出したり、確実に配信したりするために、これらをメッセージのオーディエンスに追加します。
...
詳しくは、[ターゲットマッピング](../../delivery/using/steps-defining-the-target-population.md#select-a-target-mapping)を参照してください。
+++
...
詳しくは、[ターゲティングディメンション](../../workflow/using/targeting-data.md)を参照してください。
+++
...
詳しくは、[タスクアクティビティ](../../workflow/using/task.md)を参照してください。
+++
...
タイポロジの詳細は[こちら](../../campaign-opt/using/about-campaign-typologies.md#typologies)。
+++
...
ワークフロージャーナルの詳細は[こちら](../../workflow/using/monitoring-workflow-execution.md)。
+++
