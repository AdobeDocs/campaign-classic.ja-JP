...
コストカテゴリとは、配信タイプ（電子メール、ダイレクトメールなど）の対象となる一連のコストを含むエンティティです。 スケジュールを設定できます。 コストカテゴリは、サービスプロバイダーに関連付けられているサービスのテンプレートにまとめられます。 各サービスプロバイダーは、1 つ以上のサービステンプレートを参照できます。
...
1. プロセスのタイプ（ダイレクトメール／電子メールなどによる配信またはタスク）ごとにコストカテゴリを作成します。 これを行うには、「**[!UICONTROL コストカテゴリ]**」タブをクリックしてから「**[!UICONTROL 追加]**」ボタンをクリックして、各コストカテゴリのパラメーターを入力します。
   ...
   * このコストカテゴリのラベルを入力して、該当するプロセスのタイプ（「**[!UICONTROL ダイレクトメール]**」、「**[!UICONTROL 電子メール]**」、「**[!UICONTROL モバイル]**」、「**[!UICONTROL 電話]**」または「**[!UICONTROL タスク]**」による配信）を選択します。
...
キャンペーンでは、予算を選択する際に、初期金額を入力する必要があります。 計算されたコストは、入力された金額（発生した費用、想定される費用、予約済みの費用、割り当てられた費用）のコミットメントレベルに応じて、自動的に更新されます。 [金額の計算](../../mrm/using/controlling-costs.md#calculating-amounts)を参照してください。
