...
>[!CAUTION]
>
>**DCEは、読み込まれたHTML ページに対してJavaScript コードを実行します。** Adobe Campaign インターフェイスに表示されるHTML テンプレートのJavaScript エラー。 これらのエラーは、エディターとは関係ありません。 インポートしたファイルにエラーがないことを確認するには、ファイルを DCE にインポートする前に web ブラウザーでテストすることをお勧めします。
...
1. 「**[!UICONTROL 送信」を選択します。]** **[!UICONTROL コレクション]** ページのボタン。
