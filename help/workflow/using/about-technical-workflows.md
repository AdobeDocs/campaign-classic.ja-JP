...
| **配信品質の更新**（deliverabilityUpdate） | 配信 | 配信品質の監視（メール配信品質）パッケージがインストールされると、このワークフローは夜間に実行され、バウンスメールの選定ルールのほか、ドメインと MX のリストを管理します。それには、プラットフォームで HTTPS ポートを開く必要があります。 |