...
* Web フォームが E メールで送信したリンクでアクセスできる場合で、アプリケーションにアクセスするユーザーが既にデータベースにある場合、「**[!UICONTROL プリロードされたレコードを更新]**」オプションをチェックできます。詳しくは、[E メールによるフォームの配信](publishing-a-web-form.md#delivering-a-form-via-email)を参照してください。
   ...
   >[!CAUTION]
   >
   >このオプションは、入力するフィールドがある場合、電子メールアドレスを含むユーザーデータを上書きします。 新しいプロファイルを作成するためには使用できず、フォームのプリロードボックスの使用が必要です。
...
デフォルトでは、データは **[!UICONTROL 更新または挿入]** 操作：データベース内に存在する場合は、要素が更新されます（例えば、選択したニュースレターや入力した E メールアドレスなど）。 存在しない場合、情報は追加されます。
