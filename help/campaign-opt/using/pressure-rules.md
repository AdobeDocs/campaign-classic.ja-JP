...
しきい値の計算では、デフォルトで、送信済みの配信のみが考慮されます。 Campaign Classicv7 で、 **[!UICONTROL 暫定カレンダーで配信を考慮]** オプションを使用します。 オンにすると、対象の期間が 2 倍になり、過去の配信に加えて、今後予定されている配信も考慮されます。
...
1. 「**考慮する期間**」フィールドに **[!UICONTROL 7d]** と入力し、さらに「**[!UICONTROL 暫定カレンダーで配信を考慮]**」オプションを選択します。オプション：配信日から過去 7 日の間に送信された配信および配信日から 7 日後までスケジュールされている配信が計算に含められます。

   >[!AVAILABILITY]
   >このメソッドは、Campaign v8 では使用できません。
...
### 最大の重みを持つメッセージのみの送信 {#sending-only-the-highest-weighted-messages}
