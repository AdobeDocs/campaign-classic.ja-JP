...
Adobe Campaign に既に格納されている既存のレポートを使用して、データに関する記述的分析レポートを作成できます。このモードは、分析が保存されている場合、または記述的分析アシスタントを使用してレポートが作成され、アクセスするように設定されている場合に使用できます。

記述的分析の保存方法については、[分析レポートの保存](#saving-an-analysis-report)を参照してください。

記述的分析レポートを作成するには、ワークフローのトランジションまたは **[!UICONTROL ツール/記述的分析]** メニューを使用して、記述的分析アシスタントを実行する必要があります。
