...
>[!NOTE]
>
>このレポートに表示されるドメイン名は、キューブレベルで定義されます。これらの値を変更するには、「**[!UICONTROL 配信ログ（broadlogrcp）]**」キューブを編集します。詳しくは、[この節](../../reporting/using/ac-cubes.md)を参照してください。「**[!UICONTROL その他]**」のカテゴリには、特定のクラスに属さないドメイン名が含まれます。
