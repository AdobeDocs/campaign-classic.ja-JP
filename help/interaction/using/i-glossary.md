...
* **アービトラージ**：環境上で表示されるオファー（実施要件を満たすオファー）の選択。 アービトラージの原則により、各オファーはカテゴリ、オファーおよびコンテキストオファーで定義されている基準に基づいた優先順にランキングされます。
...
* **アウトバウンドインタラクション**：（メールやダイレクトメールの配信に使用される）連絡先リストからインタラクションエンジンへの呼び出し。 各連絡先に同じルールとプロセスが適用されます。 通常、このタイプのインタラクションはバッチモードで処理されます。
