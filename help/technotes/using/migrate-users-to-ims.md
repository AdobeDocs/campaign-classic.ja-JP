...
この移行の詳細については、を参照してください。 [このページ](ac-ims.md).
...
この変更は、Campaign Classic v7.4.1 （および最新）以降に適用されます [IMS 移行互換バージョン](ac-ims.md#ims-versions)）および **mandatory** をAdobe Campaign v8 に移行します。
...
1. Adobeは、環境を Campaign v7.4.1 （または [IMS 移行互換バージョン](ac-ims.md#ims-versions)）に設定します。
...
## ハイブリッド環境およびオンプレミス環境を移行する方法 {#ims-migration-procedure-on-prem}

この移行の主な手順を以下に示します。

1. 環境を Campaign v7.4.1 にアップグレード（または [IMS 移行互換バージョン](#ims-versions)）に設定します。
1. アップグレード後も、ネイティブユーザーまたは IMS の両方の方法で新しいユーザーを引き続き作成できます。
...
Adobeは、Campaign Classic v7.4.1 にアップグレードした後は、IMS ユーザーのみを作成することをお勧めします（または [IMS 移行互換バージョン](#ims-versions)）に設定します。
Campaign v7.4.1 以降では、以下に説明するようにインスタンス設定を更新することで、ネイティブオペレーターの作成を防ぐことができます。 [このページ](impact-ims-migration.md).
...
>[!MORELIKETHIS]
>
>* [Adobe Developer コンソールへのテクニカルユーザーの移行](ims-migration.md)
