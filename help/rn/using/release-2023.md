...
* Campaignを7.3.1にアップグレードした後のHTTP プロキシ認証に関する問題を修正しました。 Campaign ワークフローのHTTP リクエストが`error 407 – proxy auth required is returned`で失敗しました。 （NEO-49624）
...
レポートの書き出し時に日本語の文字が正しく表示されない問題を修正しました（NEO-49308）。
...
* メールとは異なる外部アカウントを使用した配信で、クリック追跡データが同期されない可能性がある問題を修正しました。（NEO-47277）
