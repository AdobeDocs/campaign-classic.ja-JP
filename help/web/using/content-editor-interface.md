...
1. ウィンドウの&#x200B;**左側**&#x200B;のセクションは、コンテンツを編集するための領域です。 この領域から、ユーザーはポップアップツールバーを使用してコンテンツと直接やり取りできます。画像へのリンクの挿入、フォントの変更、フィールドの削除など。詳しくは、[&#x200B; フォームの編集](editing-content.md#editing-forms)を参照してください。
1. ウィンドウの&#x200B;**右側**&#x200B;のセクションは、コントロールパネル領域です。 この領域では、エディターの様々なオプション、特にブロックのページ見出しと一般的なオプションの設定に関連するオプションをグループ化します。境界線の追加、入力ゾーンへのデータベースフィールドのリンク、Web ページプロパティへのアクセスなど。詳しくは、[&#x200B; グローバルオプション &#x200B;](#global-options)および[&#x200B; コンテンツの編集](editing-content.md)の節を参照してください。
