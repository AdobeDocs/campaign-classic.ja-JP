...
* RHEL ディストリビューションでは、**xalan-c**&#x200B;が必須です。 `epel-release`経由でインストール：
...
### PDF レポートの書き出しの依存関係 {#pdf-export-dependencies}

**wkhtmltopdf** ツールは、レポートをPDF形式に書き出す際に使用されます。 次のパッケージが必要です。

* **libjpeg**
* **xvfb**

PDF レポートの書き出しを使用しない場合、これらのパッケージをインストールする必要はありません。
