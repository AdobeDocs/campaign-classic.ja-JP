...
* **[!UICONTROL いいえ。 共有アクティビティ数]**：それぞれのソーシャルネットワークで共有されたメッセージの合計数。この値は、対応する「**[!UICONTROL ソーシャルネットワークへの共有のリンク]**」のパーソナライゼーションブロックのアイコンでのクリック総数と等しくなります。
...
>[!NOTE]
>
>ブラウザ名とオペレーティングシステム名は、メッセージが開かれたブラウザのユーザエージェントから送信される情報の一部です。 Adobe Campaign は、そのデバイス情報を使用してデバイスのタイプを推測します。