...
Webインターフェイスは、キャンペーンの注文時にローカルエンティティが入力するフォームに対応します。

テンプレートから作成したキャンペーンに適用するWebインターフェイスのタイプを選択します。

![](assets/mkt_distr_1.png)

次の4種類のWebインターフェイスを使用できます。

* **[!UICONTROL 概要]**：ローカルエンティティは、キャンペーンの設定に関する説明を入力します。オーダーが承認されたら、セントラルエンティティがキャンペーン全体を設定および実行します。

   ![](assets/mkt_distr_6.png)

* **[!UICONTROL フォーム]** :ローカルエンティティはWebフォームにアクセスでき、Webフォームでは、使用するテンプレートに応じて、コンテンツ、ターゲット、最大サイズを編集でき、パーソナライゼーションフィールドを使用して作成日や抽出日を設定できます。ローカルエンティティは、このWebフォームからターゲットとプレビューのコンテンツを評価できます。

   ![](assets/mkt_distr_8.png)

   提供されるフォームはWebアプリケーションで指定されます。Webアプリケーションでは、テンプレートの&#x200B;**[!UICONTROL 詳細キャンペーン設定の**[!UICONTROL  Webインターフェイス&#x200B;]**フィールドのドロップダウンリストで選択する必要があります。]**&#x200B;リンク。 [ローカルキャンペーン（フォーム）の作成](../../campaign/using/examples.md#creating-a-local-campaign--by-form-)を参照してください。

   >[!NOTE]
   >
   >この例で使用されるWebアプリケーションは一例です。 フォームを使用するには、特定のWebアプリを作成する必要があります。 [API](../../configuration/using/about-web-services.md) を参照してください。
...
1. 通知メッセージの「**[!UICONTROL キャンペーンをオーダー]**」をクリックするか、Adobe Campaign の該当するボタンをクリックします。
   ...
   >[!NOTE]
   >
   >Web アプリケーションの詳細は[このセクション](../../web/using/about-web-applications.md)に記載されています。
