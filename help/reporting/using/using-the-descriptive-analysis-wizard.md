...
記述的分析アシスタントは、**[!UICONTROL ツール/記述的分析]** メニューから起動できます。この場合、分析はデフォルトで受信者に関係します（**nms:recipient**）。 分析は、Adobe Campaign データベース内のすべてのデータに適用されます。

![](assets/reporting_descriptive_wz_launch.png)

標準の受信者テーブル（**nms:recipient**）以外のテーブルを分析するには、アシスタントの最後のステージにある **[!UICONTROL 詳細設定…]** リンクをクリックし、設定に一致するテーブル（この場合は **cus:individual** を選択します。
...
リスト、ワークフローのトランジション、1 つ以上の配信、一連の受信者など、様々なコンテキストから記述的分析アシスタントを使用できます。

受信者テーブルを指す Adobe Campaign ツリーの複数のノードからアクセスできます。

項目を選択して右クリックして、記述的分析アシスタントを開きます。 選択したデータのみ分析されます。
...
記述的分析アシスタントを使用してデータを分析する場合、選択した設定によって使用できるオプションが異なります。 次に、それらについて説明します。
...
記述的分析アシスタントの最初の手順では、データをグラフ形式のみでテーブルなしで表示するように選択できます。 その場合は、グラフを設定する際に変数選択をおこなう必要があります。まず、表示する変数の数を選択し、関連するデータベースのフィールドを選択します。
...
記述的分析アシスタントを使用すると、データに関する複数のタイプの統計を計算できます。 デフォルトでは、単純カウントのみ設定されています。
...
アシスタントの最後の手順では、設定したとおりにレポート（テーブルまたはグラフ）を表示できます。
...
記述的分析アシスタントを使用して定量的レポートを作成する際に使用できるオプションについて、以下で詳しく説明します。
