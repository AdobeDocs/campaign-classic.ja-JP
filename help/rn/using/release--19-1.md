...
* 接続エラーが発生した後にSMPPコネクタの非アクティブ化が発生し、他のSMS配信が送信されず、パフォーマンスの問題が発生する問題を修正しました。
* ワークフローアクティビティを使用して説明レポートを生成する際に、誤った割合が表示される問題を修正しました。（NEO-14314）
* 「**配信**&#x200B;中の重複アドレスを除外」オプションが選択されていない場合の配信準備の問題を修正しました。 （NEO-13240）
...
* 9032を構築する場合にアップグレード後、**NoNull**&#x200B;関数がOracleデータベースで動作しない問題を修正しました。 （NEO-26488）
...
* ログファイルの空行の管理に関連する問題を修正し、MTAプロセスの動作に失敗し、配信送信でパフォーマンスが低下する原因となっていました。
