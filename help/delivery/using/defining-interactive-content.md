...
Adobe Campaignではインタラクティブ [AMP for Email](https://amp.dev/ja/about/email/) 形式：特定の条件下での動的な E メールの送信を可能にします。

AMP for Email を使用して、次のことができます。
* 適切に設定された特定のアドレスに対する AMP メールの配信をテストする
* 対応するプロバイダーに登録した後、AMP E メールを Gmail または Mail.ru アドレスに配信します。
...
現在、この形式のテストをサポートしている E メールプロバイダーは 2 つあります。Gmail および Mail.ru。

Gmail アカウントに対して AMP フォーマットで配信をテストするために必要なすべての手順と仕様については、 [Gmail](https://developers.google.com/gmail/ampemail?)、および [Mail.ru](https://postmaster.mail.ru/amp) 開発者向けドキュメント。

特に、次の要件を満たす必要があります。
* 固有の AMP セキュリティ要件に従う [Gmail](https://developers.google.com/gmail/ampemail/security-requirements)、および [Mail.ru](https://postmaster.mail.ru/amp/?lang=en#howto).
...
また、 [Gmail のヒントと既知の制限事項](https://developers.google.com/gmail/ampemail/tips) ドキュメント。
...
>[!NOTE]
>
>AMP フォーマットのテストは、Gmail と Mail.ru でのみサポートされています。

Gmail の場合、まずターゲットとする Gmail アカウント用にAdobe Campaignから配信するために使用する送信者のアドレスをに追加する必要がありま許可リストす。
...
詳しくは、 [Gmail](https://developers.google.com/gmail/ampemail/testing-dynamic-email) 開発者向けドキュメント。
...
>[!NOTE]
>
>AMP フォーマットは、Gmail および Mail.ru でのみサポートされます。

いくつかのアドレスでテストが完了すると、任意の Gmail アドレスに AMP E メールを送信できます。 これをおこなうには、Googleに登録し、回答を待つ必要があります。 次に示す手順に従います： [Gmail](https://developers.google.com/gmail/ampemail/register) 開発者向けドキュメント。 登録が完了すると、許可された送信者になります。
