...
* トラッキング署名機能が改善され、サードパーティツール（メールクライアント、インターネットブラウザーなど）による特殊文字の処理方法に関連するエラーを防止できるようになりました。URL パラメーターがエンコードされるようになりました。
...
<p>詳しくは、<a href="../../workflow/using/change-data-source.md">詳細ドキュメント</a>を参照してください。</p>
</td>
</tr>
</tbody>
</table>
...
* 完全なデータベース接続の詳細を返す **xtk:session#GetCnxInfo** API メソッドへのアクセスは、管理者ユーザーのみに制限されるようになりました。（NEO-27779）
...
* Web アプリケーション使用時に、トラッキングタグが **nms:trackingURL** スキーマに誤ったプライマリキーを生成する問題を修正しました。（NEO-27931）
