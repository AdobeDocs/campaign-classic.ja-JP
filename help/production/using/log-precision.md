...
1. 最大数のログを保存できる **TraceFilter** モード。 アクティブにするには、次のコマンドを使用します。
   ...
   >[!NOTE]
   >
   >**tracefilter:&#42;**&#x200B;を使用すると、すべてのログタイプがアクティブ化されます：ncm、rdr、nms、jst、タイミング、wdbc、ldap、soap、xtk、xtkquery、session、xtkwriter、network、pop3、inmail\
   >最も役立つログの種類は次のとおりです。 **wdbc** (すべてのSQLクエリを表示)、 **soap** （すべてのSOAP呼び出しを表示）、 **ldap** (認証後にすべてのLDAPクエリを表示)、 **xtkquery** (すべてのquerydefのリストを表示)。\
   >個別に使用できます（例：**tracefilter:soap,wdbc**）。 それらをすべてアクティブ化し、特定の他のユーザーを除外することもできます：**-tracefilter:&#42;,!soap**
