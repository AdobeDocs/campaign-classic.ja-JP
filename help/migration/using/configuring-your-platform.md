...
product: campaign
title: 設定の適応
description: Campaign v7 への移行前と移行後に設定を適応させる方法について説明します
feature: Upgrade
audience: migration
...
Adobe Campaign v7 で行われた大きな変更には、特別な設定が必要なものがあります。 これらの設定は、移行前または移行後に必要になる場合があります。

移行中、**NmsRecipient** テーブルはスキーマ定義から再構築されます。 このテーブルの SQL 構造に対してAdobe Campaign以外で行った変更は、すべて失われます。

チェックする要素の例：

* 列（またはインデックス）を **NmsRecipient** テーブルに追加したが、スキーマで詳細に説明していない場合、これは保存されません。
* **tablespace** 属性は、デフォルトで値（デプロイメント・ウィザードで定義された値）を取り戻します。
* **NmsRecipient** テーブルに参照ビューを追加した場合は、移行前に参照ビューを削除する必要があります。


## 移行前 {#before-the-migration}

Adobe Campaign v7 に移行する際には、次の設定を行う必要があります。 これらの要素は、**アップグレード後** を開始する前に対処する必要があります。
...
* パスワード
  ...
  **Admin** パスワードと **Internal** パスワードを設定する必要があります。 [詳細情報](../../migration/using/before-starting-migration.md#user-passwords)
...
**postupgrade** を実行した後、次の要素を確認して設定します。

* ミラーページ

  v6.x で、ミラーページのパーソナライゼーションブロックが変更されました。この新しいバージョンでは、これらのページにアクセスする際のセキュリティが向上しています。

  メッセージで v5 パーソナライゼーションブロックを使用すると、ミラーページの表示が失敗します。 Adobeでは、メッセージにミラーページを挿入する際に、新しいパーソナライゼーションブロックを使用することを強くお勧めします。

  ただし、一時的な回避策として（またミラーページがまだ有効な状態として）、古いパーソナライゼーションブロックに戻ってこの問題を回避できます。これを回避するには、オプション **[!UICONTROL XtkAcceptOldPasswords]** を変更して、**[!UICONTROL 1]** に設定します。 これは、新しい v6.x パーソナライゼーションブロックの使用には影響しません。

* 構文

  構文に関連するエラーが発生した場合は、アップグレード後に **serverConf.xml** ファイルの **allowSQLInjection** オプションを一時的に有効にする必要があります。これにより、コードを書き換える時間が確保されます。 コードを適合させたら、セキュリティを再アクティブ化します。
...
* Tomcat
  ...
  インストールフォルダーをカスタマイズしている場合は、移行後に、そのフォルダーが正しく更新されていることを確認してください。

* レポート

  すべての標準レポートは、現在、v6.x レンダリングエンジンを使用しています。 レポートにJavaScript コードを追加してある場合は、一部の要素に影響する可能性があります

* Web アプリケーション

  アップグレード後、識別した Web アプリケーションへの接続に問題がある場合は、**serverConf.xml** ファイルで **allowUserPassword** オプションと **sessionTokenOnly** オプションを有効にする必要があります。 セキュリティ上の問題を回避するには、問題が解決した後に、これらの 2 つのオプションを再アクティブ化する必要があります。

  Web アプリケーションのタイプとその設定に応じて、追加の操作を実行して、正しく動作することを確認する必要があります。
...
* インタラクション
  ...
  **インタラクション** を使用する場合は、移行後にパラメーターを調整する必要があります。
