...
* Adobe Campaign データベース： 多くの場合はこちらが該当します。例えば「受信者のパーソナライゼーションフィールド」は、 標準フィールド（通常、姓、名、住所、町、生年月日など）を含め、受信者テーブルで定義されているすべてのフィールドです。 ユーザー定義フィールドなどで使用できます。
...
これにより、大量のデータを処理する場合（特にパーソナライゼーションデータが FDA を通じて外部テーブルから提供される場合）、配信分析のパフォーマンスを大幅に向上させることが可能です。 詳しくは、[外部データベースへのアクセス（FDA）](../../installation/using/about-fda.md)を参照してください。
...
「**受信者プロファイル読み込みクエリを編集**」をクリックすると、ターゲティングディメンションのプロファイルクエリにデータを追加できる可能性があります。 直接抽出されたデータを式内で使用して、本当に必要なものを抽出できます。 それらのデータはコンテキストに追加されます。

![](assets/persotab-edit.png){zoomable="yes"}

追加するデータがターゲティングディメンションにない場合は、「**スクリプトの実行コンテキストにオブジェクトを追加**」をクリックします。

![](assets/persotab_addobjects.png){zoomable="yes"}

必要なオブジェクトスキーマをそこで選択し、JavaScript コード内でそれらのデータを使用できます。このコードは、配信の準備中に実行されます。
