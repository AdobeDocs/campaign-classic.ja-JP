...
マーケティング分析では、レポートを容易に作成し、キューブを介してデータベースからデータを特定して選択できます。 これにより、次のことが可能になります。

* キューブに基づくレポートの作成。 手順について詳しくは、[レポートのデータを調べる](#exploring-the-data-in-a-report)を参照してください。
* データベース内のデータを収集し、リストにグループ化することで、例えばターゲットと配信を識別して作成します。 詳しくは、[ターゲット母集団の作成](#building-a-target-population)を参照してください。
* レポートにピボットテーブルを挿入し、その中で既存のキューブを参照します。 詳しくは、[ピボットテーブルをレポートに挿入する](#inserting-a-pivot-table-into-a-report)を参照してください。
...
### 手順 1 - キューブに基づいてレポートを作成する {#step-1---creating-a-report-based-on-a-cube}

キューブに基づいてレポートを作成するには、「**[!UICONTROL レポート]**」タブの「**[!UICONTROL 作成]**」ボタンをクリックして、使用するキューブを選択します。

プロセスについて詳しくは、[キューブに基づいてレポートを作成する](../../reporting/using/creating-indicators.md#creating-a-report-based-on-a-cube)を参照してください。

### 手順 2 - 行と列を選択 {#step-2---selecting-lines-and-columns}
...
1. 表の行と列に表示する寸法を選択します。 これを行うには、次に示すように、使用可能なディメンションをドラッグ&amp;ドロップします。
...
### 手順 3 - 表示する測定値を設定 {#step-3---configuring-the-measures-to-display}

行と列を適切に配置したら、表示する測定とそれらの表示モードを指定します。

デフォルトでは、1 つの測定のみ表示されます。 メジャーを追加または設定するには：
...
1. 「**[!UICONTROL 測定を使用]**」ボタンをクリックすると、既存の測定のいずれかを選択できます。
   ...
   表示する情報と書式設定の種類を選択します。 オプションのリストは、設定された測定のタイプによって異なります。
...
それには、データを買い物かごに追加し、買い物かごの内容を処理します。
...
1. エクスポートが完了したら、実行を確認するメッセージおよび処理されたレコードの数が表示されます。
   ...
   関係するリストには、「**[!UICONTROL プロファイルとターゲット]**」タブからアクセスします。

   ![](assets/s_advuser_cube_in_report_config_02g.png)

## レポートにピボットテーブルを挿入する {#inserting-a-pivot-table-into-a-report}
