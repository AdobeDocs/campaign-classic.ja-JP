...
移行中、 **NmsRecipient** テーブルがスキーマ定義から再構築されます。 このテーブルの SQL 構造に対してAdobe Campaign以外で行った変更は、すべて失われます。

チェックする要素の例：

* に列（またはインデックス）を追加した場合 **NmsRecipient** テーブルですが、スキーマで詳細に説明していません。これは保存されません。
* この **テーブルスペース** 属性は、デフォルトで値（デプロイメントウィザードで定義された値）を取り戻します。
* 参照ビューをに追加した場合 **NmsRecipient** テーブル。移行前に削除する必要があります。


## 移行前 {#before-the-migration}

Adobe Campaign v7 に移行する際には、次の設定を行う必要があります。 これらの要素は、開始する前に対処する必要があります **アップグレード後**.
...
* パスワード
  ...
  を設定する必要があります **Admin** および **内部** パスワード。 [詳細情報](../../migration/using/before-starting-migration.md#user-passwords)
...
* ミラーページ
  ...
  ただし、一時的な回避策として（またミラーページがまだ有効な場合は）、古いパーソナライゼーションブロックに戻すと、オプションを変更してこの問題を回避できます **[!UICONTROL XtkAcceptOldPasswords]** およびを設定します **[!UICONTROL 1]**. これは、新しい v6.x パーソナライゼーションブロックの使用には影響しません。

* 構文

  構文に関連するエラーが発生した場合は、アップグレード後に、を一時的にアクティブ化する必要があります **allowSQLInjection** のオプション **serverConf.xml** ファイルを開きます。これにより、コードを書き換える時間を確保できます。 コードを適合させたら、セキュリティを再アクティブ化します。
...
* レポート
  ...
  すべての標準レポートは、現在、v6.x レンダリングエンジンを使用しています。 レポートに JavaScript コードを追加している場合、一部の要素に影響する可能性があります。

* Web アプリケーション

  アップグレード後に、特定した Web アプリケーションへの接続に問題がある場合は、をアクティブ化する必要があります **allowUserPassword** および **sessionTokenOnly** のオプション **serverConf.xml** ファイル。 セキュリティ上の問題を回避するには、問題が解決した後に、これらの 2 つのオプションを再アクティブ化する必要があります。
...
* インタラクション
  ...
  を使用する場合 **相互作用**&#x200B;の場合、移行後にすべてのパラメーターを調整する必要があります。
