...
この領域の使用または監視が適切におこなわれないと、サーバー上の使用可能な物理領域がすぐにいっぱいになって、その後のアップロードでファイルが切り捨てられる可能性があります。アドビがホストする SFTP サーバーでは、SFTP ストレージが 80％のしきい値に達するとファイルが圧縮されます。このプロセスは自動で、Adobe モニタリングシステムによってトリガーされます。
