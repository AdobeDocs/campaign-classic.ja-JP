...
E メールが失敗すると、リモートサーバーは、 [プラットフォームパラメーター](../../installation/using/deploying-an-instance.md). Adobe Campaign は、各バウンスメールのコンテンツをルールのリストの文字列と比較して、3 つの[エラータイプ](#delivery-failure-types-and-reasons)のいずれかを割り当てます。
