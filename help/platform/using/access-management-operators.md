...
Adobe Campaignでは、管理者（「管理者」）、請求（「請求」）、モニタリング、Web アプリケーションエージェント（「webapp」）などのデフォルトで設定されたプロファイルを使用して、テクニカルオペレーターを使用します。これらの一部は、プラットフォームにインストールされているアプリケーションやオプションに依存します。例えば、「central」および「local」演算子は、Distributed Marketing オプションがインストールされている場合にのみ表示されます。
