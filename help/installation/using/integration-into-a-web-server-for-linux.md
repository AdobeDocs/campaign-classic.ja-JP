...
1. フォルダーにAdobe Campaign固有の設定ファイルを作成し `/etc/httpd/conf.d/` ます。 次に例を示します。`CampaignApache.conf`

1. RHEL7 **の場合**、次の手順をファイルに追加します。
...
WebブラウザーからURLをリクエストす [`https://<computer>`](https://machine/r/test) ることもできます。