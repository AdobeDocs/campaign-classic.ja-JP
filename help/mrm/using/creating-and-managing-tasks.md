...
すべてのタスクは、「**キャンペーン** 」タブからアクセスできるリストにグループ化されています。 詳しくは、[アクセスタスク](#accessing-tasks)を参照してください。
...
## 新しいタスクを作成します {#creating-a-new-task}
...
タスクにドキュメントやマーケティングリソースを追加できます（詳しくは、[マーケティングリソースの管理](../../mrm/using/managing-marketing-resources.md)を参照）。 追加するには、タスクを開き、タスクツールバーの&#x200B;**[!UICONTROL ドキュメント]**&#x200B;アイコンをクリックします。
...
>[!NOTE]
>
>予算とコストの管理方法については、[管理コスト](../../mrm/using/controlling-costs.md)で説明しています。
...
1. 担当者がタスクを作成し、複数のフィールドを入力します。詳しくは、[新しいタスクの作成](#creating-a-new-task)を参照してください。
