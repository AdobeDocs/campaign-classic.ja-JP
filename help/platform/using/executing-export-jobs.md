...
書き出しアシスタントを使用すると、書き出しを設定し、そのオプションを定義して、実行を開始できます。 これは、エクスポートのタイプ（簡易または複数）やオペレーターの権限によって内容の異なる一連の画面です。

新しいエクスポートジョブを作成すると、エクスポートアシスタントが表示されます（[ インポートジョブとエクスポートジョブの作成 ](../../platform/using/creating-import-export-jobs.md)。
...
フィルター条件を追加して、すべてのデータがエクスポートされないようにすることができます。このフィルタリングの設定は、配信アシスタントでの受信者のターゲティングと同じです。 [このページ](../../delivery/using/steps-defining-the-target-population.md)を参照してください。
...
アシスタントの下部にあるタブをクリックして、列での結果のプレビューから XML の結果に切り替えます。 生成された SQL クエリを表示することもできます。
