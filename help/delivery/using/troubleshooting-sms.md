...
プロバイダーは、`BIND PDU`の送信元の外部アカウントを区別する手段がないので、複数のアカウントからのすべての接続を単一のアカウントとして扱います。MOとSRが2つのアカウントをランダムにルーティングし、問題が発生した可能性があります。
プロバイダーが同じログインとパスワードの組み合わせに対して複数のショートコードをサポートしている場合は、そのショートコードをどこに`BIND PDU`に入れるかを尋ねる必要があります。この情報は`BIND PDU`がMOのルーティングを正しく許可する唯一の場所であるため、`SUBMIT_SM`ではなく`BIND PDU`内に配置する必要があることに注意してください。
上記の各タイプのPDU](sms-protocol.md#information-pdu)の[情報を参照して、`BIND PDU`で使用できるフィールドを確認してください。通常は`address_range`に短いコードを追加しますが、これにはプロバイダーからの特別なサポートが必要です。複数のショートコードを個別にルーティングする方法については、担当者にお問い合わせください。
Adobe Campaignでは、同じ外部アカウントでの複数のショートコードの処理がサポートされています。
...
* `BIND PDU`さんがたくさんいます。 接続の数に応じて、1日に数個を超えてはいけません。 1 時間に 1 つ以上の BIND PDU は、警告を発する必要があります。
...
* 電話番号の形式が、強制隔離テーブルと配信ログで完全に同じであることを確認します。 それ以外の場合で、国際電話番号形式のプラス接頭辞に問題がある場合は、この[節](sms-protocol.md#automatic-reply)を参照してください。
...
* タイミングの問題（`retry timing`、`enquire_link` 期間、スループットのキャップなど）
...
**送信済み**、**プロバイダーに送信済み**、**モバイルで受信済み**&#x200B;のステータスの違いを明確にするには、以下の詳細な定義を参照してください。

* **モバイルで受信**:
メッセージがユーザーのデバイスに正常に配信され、Mobile Terminated （MT）配信とStatus Report （SR）の両方から確認が提供されました。

* **送信済み**:
メッセージはMobile Terminated （MT） ステップを通じて正常に処理されましたが、モバイルデバイスへの配信を確認するStatus Report （SR）はまだ受信されていません。

* **プロバイダーに送信**:
メッセージは`SUBMIT_SM command`を使用してプロバイダーに送信されましたが、プロバイダーから`SUBMIT_SM_RESP`確認は受信されませんでした。
