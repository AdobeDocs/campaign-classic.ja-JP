...
>[!NOTE]
>
>Adobe Campaign Standard の **SMS コネクタのプロトコルと設定**&#x200B;については、この[ページ](https://experienceleague.adobe.com/docs/campaign-standard/using/administrating/configuring-sms/sms-protocol.html?lang=ja#administrating)を参照してください。
>
...
SMS は、テキストよりも多くの情報を伝送します。SMSによって提供される情報のリストを以下に示します。
...
[SMPP プロトコル仕様](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)（167 ページ）の付録 B の節に推奨事項が記載されていますが、実際のエラーコードとその意味は記載されていません。
...
この設定を調整する場合は、誤ったトリガーを避けるために、できる限り多くのコンテキストを含めてください。標準に `id:` のような特定のプリフィックスがある場合は、正規表現に含めます。また、単語の途中でテキストが取り込まれないように、できる限り単語区切り文字（\b）を使用します。
...
* **SMS にさまざまな文字を送信する**：GSM 以外の文字や ASCII 以外の文字で SMS を送信する必要がある場合は、できるだけ多様な文字でメッセージを送信してみてください。カスタム文字マッピングテーブルを設定する場合は、 
