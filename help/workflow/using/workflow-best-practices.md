...
ログが不要になった場合は、ログを消去します。ワークフロー履歴は自動的には消去されません。すべてのメッセージはデフォルトで保持されます。履歴は、**[!UICONTROL ファイル/アクション]** メニューを使用するか、リストの上にあるツールバーにある「アクション」ボタンをクリックすることで消去できます。「履歴を消去」を選択します。
ログのパージ方法については、この[&#x200B; ドキュメント &#x200B;](starting-a-workflow.md)を参照してください。
