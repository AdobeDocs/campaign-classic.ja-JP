...
キャンペーンプロセスを完全にモニタリングおよび制御するために、配信の各手順は承認の対象となることがあります。 これには、ターゲティング、コンテンツ、予算、抽出、配達確認の送信が含まれます。

レビュー担当者に指名されている [!DNL Adobe Campaign] オペレーターに通知メッセージが送信されて、承認リクエストを通知します。 レビュー担当者が承認の&#x200B;**適切な権限**&#x200B;を持っていること、およびレビュー担当者のセキュリティゾーンが正しく定義されていることを確認します。 [レビュー担当者の選択の詳細情報](#selecting-reviewers)

承認手順について詳しくは、[承認手順の概要](#checking-and-approving-deliveries)を参照してください。
...
![検証リンクを含む承認通知メール](assets/s_user_validation_link_in_mail.png)

レビュー担当オペレーターは、予算を承認するかどうかを選択できます。

![許可または却下オプションを含む承認確認ページ](assets/s_user_validation_page_confirm.png)

オペレーターによる検証が完了すると、ジョブの承認または却下が配信ダッシュボードに転送されます。

![ジョブの承認リンクを示すキャンペーンダッシュボード](assets/s_user_validation_link_in_op_board.png)

この情報は、キャンペーンの承認ログにも表示されます。 これらのログには、**[!UICONTROL 編集／トラッキング／承認]**&#x200B;タブからアクセスします。

![承認ログを示すキャンペーン編集タブ](assets/s_user_validation_log_in_op_edit_tab.png)

これらの通知は、承認を有効にした各プロセスに影響を与えるオペレーターに送信されます。

承認は、キャンペーンテンプレート、個々の各キャンペーンまたは配信に対して有効にできます。

承認が必要なすべてのジョブは、キャンペーンテンプレート（**[!UICONTROL プロパティ]**／**[!UICONTROL キャンペーンの詳細設定...]**／**[!UICONTROL 承認]**&#x200B;タブ）で選択します。 承認を担当するオペレーターもそこで選択され、このオプションが無効になっていない限り、通知を受け取ります。 詳しくは、[配信を承認する手順](#approving-processes)を参照してください。
...
![プロセスの選択を含む配信承認設定](assets/s_user_validation_select_process_from_del.png)
...
![承認オペレーターのレビュー担当者の追加ダイアログ](assets/s_user_validation_add_operator.png)

レビュー担当者を指定しない場合は、キャンペーンマネージャーが承認を担当し、通知を受け取ります。 キャンペーンマネージャーは、キャンペーンの&#x200B;**[!UICONTROL 編集／「プロパティ」]**&#x200B;タブで指定します。

![マネージャーフィールドを示すキャンペーンのプロパティ](assets/s_user_op_manager_field.png)

>[!NOTE]
>
>**[!UICONTROL 管理者]**&#x200B;権限を持つその他すべての [!DNL Adobe Campaign] オペレーターもジョブを承認できますが、通知は受け取りません。\
...
![コンソールでのキャンペーンダッシュボードの承認アクション](assets/s_user_validation_from_console.png)
...
![許可または却下とコメントを含む承認ページ ](assets/s_user_validation_save_target_validation.png)
...
![手動モードと自動モードを含む承認設定](assets/s_user_validation_select_mode.png)
...
![無効な通知を含む承認設定](assets/s_user_validation_notif_desactivate.png)

### 通知コンテンツ {#notification-content}

通知コンテンツは、特定のテンプレート「**[!UICONTROL マーケティングキャンペーンの検証の通知]**」で定義されます。 このテンプレートは、[!DNL Adobe Campaign] ツリーの&#x200B;**[!UICONTROL 管理／キャンペーン管理／テクニカル配信テンプレート]**&#x200B;フォルダーに保存されています。

## 配信のレビューと承認 {#checking-and-approving-deliveries}

[!DNL Adobe Campaign] では、マーケティングキャンペーンのメインステージの承認プロセスを協調モードで設定できます。

ダイレクトメール配信の場合、[!DNL Adobe Campaign] のオペレーターは、抽出ファイルを発送担当に送信する前に表示できます。また、必要に応じて書式設定を変更し、抽出を再開できます。 [抽出ファイルの承認](#approving-an-extraction-file)を参照してください。

キャンペーンごとに、配信ターゲット、コンテンツ（[コンテンツの承認](#approving-content)を参照）およびコストを承認できます。 [!DNL Adobe Campaign] の承認担当オペレーターは、メールで通知を受け、コンソールまたは web 接続で承認を許可または却下できます。 [配信を承認する手順](#approving-processes)を参照してください。

この検証フェーズが完了したら、配信を開始できます。 [配信の開始の詳細情報](../../campaign/using/marketing-campaign-deliveries.md#starting-a-delivery)
...
![検証する配信ステータスを示すキャンペーンダッシュボード](assets/s_ncs_user_edit_del_to_validate.png)
...
![メッセージ準備を再開するダッシュボードリンク](assets/s_user_validation_relaunch_message_preparation.png)
...
* **抽出の承認（ダイレクトメール配信）**
  ...
  ![「コンテンツを承認」リンクを示す承認ダッシュボード](assets/s_ncs_user_edit_file_valid.png)

  抽出ファイルを承認ボックスからプレビューして、許可または却下できます。

  ![承認ダイアログでの抽出ファイルのプレビュー](assets/s_ncs_user_edit_file_valid_preview_file.png)
...
承認フェーズは、キャンペーンに関連付けられたテンプレートで定義されます。 承認する要素をテンプレートから選択し、その承認を担当する [!DNL Adobe Campaign] オペレーターを指定する必要があります。 キャンペーンテンプレートについて詳しくは、[キャンペーンテンプレート](../../campaign/using/marketing-campaign-templates.md#campaign-templates)を参照してください。
...
承認するプロセスを選択／選択解除したり、承認を担当する [!DNL Adobe Campaign] オペレーターを指定したりできます。 個人のオペレーター、オペレーターのグループ、オペレーターのリストを指定できます。
...
![承認カレンダーとリマインダー設定](assets/s_ncs_user_edit_op_valid_calendar.png)

配信ごとに承認日と自動リマインダーを表示および編集するには、「**[!UICONTROL 監査]**」ボタンをクリックし、「**[!UICONTROL 承認]**」タブをクリックします。

![日付とリマインダーを含む配信承認タブ](assets/s_ncs_user_edit_del_valid.png)
...
>[!CAUTION]
>
>コンテンツを承認するには、配達確認サイクルが必須です。 配達確認を使用すると、情報やパーソナライゼーションデータの表示を承認し、リンクが機能していることを確認できます。 配達確認の作成方法について詳しくは、[配達確認の作成](../../delivery/using/steps-validating-the-delivery.md#sending-a-proof)を参照してください。
>
...
1. 新しい配信を作成した後、キャンペーンマネージャーがキャンペーンダッシュボードの「**[!UICONTROL コンテンツを送信]**」リンクをクリックして、コンテンツ承認サイクルを開始します。
   ...
   ![承認用にコンテンツを送信するキャンペーンダッシュボードリンク](assets/s_ncs_user_validation_submit_content_validation.png)
...
1. 通知メールがコンテンツの担当者に送信されます。この担当者は、コンテンツを承認するかどうかを選択できます。
   ...
   * 通知メールから：
     ...
     ![配達確認のコンテンツ承認通知メール](assets/s_ncs_user_del_content_valid_bat_notif.png)
   ...
   * コンソールまたは Web インターフェイス、配信トラッキング、配信ダッシュボードまたはキャンペーンダッシュボードから：
     ...
     ![コンテンツ配達確認リストを示す配信トラッキング](assets/s_ncs_user_validation_content_bat_op.png)
     ...
     ![コンテンツ承認用の配達確認の詳細ビュー](assets/s_ncs_user_validation_content_BAT_details.png)
...
1. コンテンツが承認されたかどうかを知らせる通知メールがキャンペーンの担当者に送信されます。
   ...
   ![コンテンツ承認を再開するキャンペーンダッシュボードリンク](assets/s_user_validation_relaunch_content_validation.png)
...
1. 新しい配信を作成した後、キャンペーン担当者がキャンペーンダッシュボードの「**[!UICONTROL コンテンツ編集を送信]**」リンクをクリックして、コンテンツ編集サイクルを開始します。
   ...
   ![コンテンツ編集を送信するキャンペーンダッシュボードリンク](assets/s_ncs_user_validation_submit_content_edition.png)
...
1. コンテンツ編集担当者はコンソールにログオン、簡素化されたアシスタントで配信を開いて編集、件名、HTML およびテキストコンテンツを変更して、配達確認を送信できます。
   ...
   ![配信コンテンツの編集用の簡素化されたアシスタント](assets/s_user_validation_content_edition.png)
...
1. 配信コンテンツの変更を完了したら、コンテンツ編集担当者はコンテンツを使用可能にすることができます。
   ...
   * [!DNL Adobe Campaign] コンソールから&#x200B;**[!UICONTROL 使用可能なコンテンツ]**&#x200B;リンクをクリックします。

     ![コンテンツを使用可能にするコンソールリンク](assets/s_ncs_user_validation_submit_content_available.png)

   * 通知メッセージ内のリンクをクリックして、コンテンツの可用性を承認します。

     ![コンテンツの可用性を承認する通知リンク](assets/s_ncs_user_validation_submit_content_available2.png)

     オペレーターは、コンテンツをキャンペーン担当者に送信する前にコメントを追加できます。

     ![コンテンツの可用性を送信する前のコメントフィールド](assets/s_ncs_user_validation_submit_content_available3.png)
...
1. 通知メールには、送信された配達確認へのリンクが含まれています。このリンクを使用して、配信レンダリングと、配信コンテンツを承認または却下するためのボタンを表示できます。
   ...
   ![配達確認リンクを含む外部コンテンツ承認メール](assets/s_user_validation_external_content.png)

### 抽出ファイルの承認 {#approving-an-extraction-file}

オフライン配信の場合は、[!DNL Adobe Campaign] により、設定に応じて、発送担当に送信する抽出ファイルが生成されます。 抽出ファイルのコンテンツは、使用するエクスポートテンプレートによって異なります。
