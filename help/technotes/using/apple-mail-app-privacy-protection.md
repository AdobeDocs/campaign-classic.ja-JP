...
IOS デバイスでAppleのメール アプリを使用するオーディエンスの割合を確認します。
この評価を利用することで、潜在的な異常値ギャップとその原因を特定できます。ギャップがキャンペーンパフォーマンスの問題によるものか、Appleのプライバシー保護機能によるものかを判断できます。[のヒントとテクニック &#x200B;](#measure-ios-footprint)をお読みください。
...
* 定義済みフィルターを使用できます。 詳しくは、[Campaign v8 ドキュメント](https://experienceleague.adobe.com/docs/campaign/automation/workflows/use-cases/designing-queries/create-a-filter.html?lang=ja){target="_blank"}を参照してください。
...
1. クエリで始まるワークフローを作成します。
   ...
   最初のクエリは、過去3か月間のトラッキングログを取得するために使用されます。
増分クエリを使用して、まだ書き出されていないレコードのみを抽出できます。
