...
# 承認プロセスのセットアップと管理{#approving-marketing-campaigns}

キャンペーンの各種プロセス（ターゲット設定、コンテンツ、予算、抽出、校正の送信）の監視と制御を完全かつ確実におこなうために、配信の各段階で承認を要求できます。

通知メッセージは、指定されたレビュー担当者であるAdobe Campaignオペレーターに送信され、承認要求を通知します。 レビュー担当者が承認の&#x200B;**適切な権限**&#x200B;を持っていること、およびレビュー担当者のセキュリティゾーンが正しく定義されていることを確認します。 [詳細情報](#selecting-reviewers)。

承認手順は、[このセクション](#checking-and-approving-deliveries)に記載されています。

>[!NOTE]
>
>配信のオーナーのみが配信を開始できます。別の演算子（または演算子グループ）を配信に開始できるようにするには、**[!UICONTROL 配信開始:]**&#x200B;フィールドにレビュー担当者として追加する必要があります。\
>[詳細情報](#selecting-reviewers)。

## 動作の仕組み {#operating-principle-}

例えば、予算の承認に関する標準メッセージは次のようになります。

![](assets/s_user_validation_link_in_mail.png)

レビュー担当者は、予算の承認を選択できます。

![](assets/s_user_validation_page_confirm.png)

オペレーターが検証を行うと、ジョブの承認または拒否が配信ダッシュボードに転送されます。

![](assets/s_user_validation_link_in_op_board.png)

この情報は、キャンペーンの承認ログにもあります。これらのログには、**[!UICONTROL 編集/追跡/承認]**&#x200B;タブからアクセスします。
...
承認を必要とするすべてのジョブをキャンペーンテンプレート内で選択します（**[!UICONTROL プロパティ]**／**[!UICONTROL キャンペーンの詳細設定...]**／「**[!UICONTROL 承認]**」タブ）。承認を担当するオペレーターも選択します（このオプションが無効でない限り、このオペレーターが通知を受け取ります）。詳しくは、[こちらの節](#approving-processes)を参照してください。
...
## レビュー担当者を選択{#selecting-reviewers}

承認のタイプごとに、承認を担当するオペレーターまたはオペレーターグループを配信のドロップダウンリストから選択します。**[!UICONTROL 編集…を使用して、さらに演算子を追加できます。]**&#x200B;リンクをクリックします。 このウィンドウでは承認期日を編集することもできます。
...
通知メッセージに表示されるリンクをクリックします（[通知](#notifications)を参照）。次に示すように、ログインする必要があります。
...
### 通知を有効/無効にする{#enabling-disabling-notification}
...
配信ターゲット、コンテンツ（[コンテンツの承認](#approving-content)を参照）、およびコストをキャンペーンごとに承認できます。 承認を担当する Adobe Campaign オペレーターは、E メールで通知を受け、コンソールから、または Web 接続を介して、承認を許可または却下できます。[配信を承認する手順](#approving-processes)を参照してください。
...
### 選択したプロセスが{#choosing-the-processes-to-be-approved}を承認

承認フェーズは、キャンペーンに関連付けられたテンプレートで定義されます。テンプレートから承認するAdobe Campaignを選択し、これらの承認を担当する要素演算子を指定する必要があります。 キャンペーンテンプレートの詳細については、[この](../../campaign/using/marketing-campaign-templates.md#campaign-templates)を参照してください。

>[!NOTE]
>
>キャンペーン(またはキャンペーンテンプレート)の承認設定は、このキャンペーンにリンクされた今後のすべての配信に適用されます。 設定の変更が以前の配信に適用されることはありません。
...
>[!NOTE]
>
>* レビュー担当者のリストが定義されている場合、あるレビュー担当者がジョブを承認すると、そのジョブが承認されます。 すると、関連する承認リンクはダッシュボードに表示されなくなります。通知の送信が有効な場合、別のレビュー担当者が通知メッセージ内の承認リンクをクリックすると、別のオペレーターが既にジョブを承認したことが通知されます。
...
>[!CAUTION]
>
>コンテンツを承認するには、配達確認サイクルが必須です。 配達確認を使用して、情報やパーソナライゼーションデータの表示を承認し、リンクが機能していることを確認できます。[このセクション](../../delivery/using/steps-validating-the-delivery.md#sending-a-proof)で配達確認を作成する方法を説明します。
>
>以下に説明するコンテンツ配達確認機能は、承認の配信に関連しています。
...
1. 通知電子メールは、コンテンツの担当者に送信されます。管理者は、コンテンツを承認するかどうかを選択できます。
...
ファイルが承認されたら、通知 E メールを発送担当に送信できます。詳しくは、[オフライン配信](../../campaign/using/marketing-campaign-deliveries.md#starting-an-offline-delivery)の開始を参照してください。
